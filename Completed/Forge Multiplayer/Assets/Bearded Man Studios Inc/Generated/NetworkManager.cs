...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlayerMoveNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlayerMoveNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerMoveNetworkObject.Length > 0 && PlayerMoveNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlayerMoveNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlayerMoveBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlayerMove instead, its shorter and easier to type out ;)")]
		public PlayerMoveBehavior InstantiatePlayerMoveNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerMoveNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerMoveBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlayerMoveBehavior>().networkObject = (PlayerMoveNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of PlayerMove
		/// </summary>
		/// <returns>
		/// A local instance of PlayerMoveBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerMove prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerMoveBehavior InstantiatePlayerMove(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerMoveNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerMoveBehavior>();
